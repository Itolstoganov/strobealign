...
void StrobemerIndex::assign_randstrobes(size_t ref_index, size_t offset) {
    ...
    while (!end) {
        ...
        for (auto randstrobe : chunk) {
            RefRandstrobe::packed_t packed = ref_index << RefRandstrobe::bit_alloc;
            ...
            packed += (strobe2_offset << (RefRandstrobe::bit_alloc / 2));
            packed += strobe3_offset;
...
void StrobemerIndex::print_diagnostics(const std::string& logfile_name, int k) const {
    ...
    log_file << "Distinct," << distinct_seeds << std::endl;
}
